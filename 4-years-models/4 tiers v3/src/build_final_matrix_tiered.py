...
OUTPUT_TRAIN = "Train.parquet"
OUTPUT_VAL   = "Validation.parquet"
...
if mask_other.any():
    # Isolate the global absolute marginal values for 'Other'
    mv_global = df_const.loc[mask_other, 'MARGINALVALUE'].abs()
    
    # Apply Binding Distribution Filter (Exclude zeros)
    binding_mv = mv_global[mv_global > 0]
    
    if not binding_mv.empty:
        # Compute exact empirical quantiles
        q99 = np.percentile(binding_mv, 99)
        q90 = np.percentile(binding_mv, 90)
        q50 = np.percentile(binding_mv, 50)
        
        print("\n[STATISTICS] Global 4-Tier Thresholds Derived:")
        print(f"  Super-High (99th): >= ${q99:.2f}")
        print(f"  High (90th):       >= ${q90:.2f}")
        print(f"  Med (50th):        >= ${q50:.2f}")
        print(f"  Low:               <  ${q50:.2f}")
        
        # Apply instantaneous mapping
...
# ==========================================
print("\n[4/4] Saving Files...")
train_df.drop(columns=['SETTLEMENTDATE']).to_parquet(OUTPUT_TRAIN, index=False)
val_df.drop(columns=['SETTLEMENTDATE']).to_parquet(OUTPUT_VAL, index=False)
